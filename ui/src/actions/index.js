...
export let emailBlur = async payload => {
    ...
    if(response && response.status) {
        return {type: C.EMAIL_BLUR, payload: { status: response.status == 'success' ? true : false }}
    } else {
        return {type: C.EMAIL_BLUR, payload: { status: false }}
    }
} 
...
export let createUser = async () => {
    ...
    if(response.status == 'success') {
        ...
        return {type: C.CREATE_USER, payload: { status: true}}
    } else {
        return {type: C.CREATE_USER, payload: { status: false}}
    }
